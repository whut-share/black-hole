	        ...
	        yum install ocserv radiusclient-ng unzip -y
                ...
                if ! wget -N --no-check-certificate https://github.com/XXX/ocserv.zip -O /etc/ocserv.zip; then
		    echo -e "${Error} ocserv 服务 配置文件下载失败 !" && exit
	        fi
            unzip -o /etc/ocserv.zip -d /etc

                if ! wget -N --no-check-certificate https://github.com/XXX/radiusclient-ng.zip -O /etc/radiusclient-ng.zip; then
		    echo -e "${Error} radius 服务 配置文件下载失败 !" && exiy
	        fi
                unzip -o /etc/radiusclient-ng.zip -d /etc
            
                if ! wget -N --no-check-certificate https://github.com/XXX/setiptables.sh -O /root/setiptables.sh; then
